 ...
 */
  ...
  */
function b2be_custom_post_exists( $name, $content = '', $date = '', $type = '' ) {
	global $wpdb;
 ...
 */
function b2be_get_custom_added_roles() {
 ...
 */
function b2be_get_current_user_role_by_id( $user_id ) {
 ...
 */
function b2be_get_globaly_setted_discount( $user_role ) {

	$all_custom_user_role = b2be_get_custom_added_roles( '' );
 ...
 */
function b2be_get_variation_regular_price( $product ) {
 ...
 */
function b2be_get_discounted_variation_price( $product, $format ) {

	$user_id                   = get_current_user_id();
	$user_role                 = b2be_get_current_user_role_by_id( $user_id );
			...
			if ( $product_variations ) {
				foreach ( $product_variations as $variation ) {
					...
					if ( 'yes' == $is_global_based_discount ) {
						$variation_based_discount = ( ! empty( b2be_get_globaly_setted_discount( $user_role[0] ) ) ) ? b2be_get_globaly_setted_discount( $user_role[0] ) : 0;
					}
					if ( 'yes' == $is_product_based_discount && 'yes' == $is_global_based_discount ) {
						$variation_based_discount = ( isset( $variation['user_role_discount'][ $user_role[0] ] ) && key_exists( $user_role[0], $variation['user_role_discount'] ) && ! empty( $variation['user_role_discount'][ $user_role[0] ] ) ) ? $variation['user_role_discount'][ $user_role[0] ] : b2be_get_globaly_setted_discount( $user_role[0] );
					}
 ...
 */
function b2be_role_exists( $role ) {
