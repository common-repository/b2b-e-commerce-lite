...
class B2BE_RFQ_Comments {
	...
	public function rfq_save_customer_comment() {
		...
		if ( ! empty( $_POST['rfq_customer_comment_textarea'] ) ) {
			...
			if ( ! is_wp_error( $inserted ) ) {
				WC()->mailer()->emails['new_comment_submitted']->trigger( b2be_get_quote( $rfq_customer_commentdata['comment_post_ID'] ) );
			}
		}
	}
	/**
	...
	public function rfq_save_admin_comment() {
		...
		if ( ! empty( $_POST['rfq_admin_comment_textarea'] ) ) {
			...
			if ( ! is_wp_error( $inserted ) ) {
				WC()->mailer()->emails['new_admin_comment_submitted']->trigger( b2be_get_quote( $rfq_admin_commentdata['comment_post_ID'] ) );
			}
		}
	}
}
