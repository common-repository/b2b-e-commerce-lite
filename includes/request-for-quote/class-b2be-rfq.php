...
if ( ! class_exists( 'B2BE_RFQ' ) ) {
	...
	class B2BE_RFQ {
		...
		public function __construct() {
			...
			if ( ( B2BE_PLUGIN_NAME !== null ) && is_admin() ) {
				add_action( 'woocommerce_init', array( $this, 'b2be_rfq_create_pages' ), 10, 1 );
			}
			...
			add_filter( 'woocommerce_account_menu_items', array( $this, 'quotes_menu_item' ) );
			...
			add_action( 'woocommerce_account_' . B2BE_VIEW_QUOTE_ENDPOINT . '_endpoint', array( $this, 'woocommerce_account_view_quote' ) );
			add_filter( 'woocommerce_endpoint_' . B2BE_VIEW_QUOTE_ENDPOINT . '_title', array( $this, 'view_quote_page_title' ), 10, 2 );
			add_action( 'b2be_rfq_view_quote', array( $this, 'woocommerce_quote_details_table' ), 10 );
		...
		public function add_to_rfq_button() {
			...
			?>
				<button type="submit" name="add-to-rfq" value="<?php echo esc_attr( $product->get_id() ); ?>" class="single_add_to_rfq_button button alt"><?php echo ( '' !== $add_to_rfq_btn_txt ) ? esc_html( $add_to_rfq_btn_txt ) : 'Add To RFQ'; ?></button>        
			<?php
		}
		 ...
		 */
		public function b2be_rfq_create_pages() {
			...
			$thankyou_page_check = get_page_by_title( 'Thank You' );
			$rfq_page_check      = get_page_by_title( 'RFQ' );
			$thankyou_page       = array(
				'post_type'    => 'page',
		...
		public function woocommerce_init() {
			...
			if ( ! is_admin() || defined( 'DOING_AJAX' ) ) {
				...
				include B2BE_PLUGIN_DIR . '/includes/request-for-quote/class-b2be-rfq-cart.php';
		...
		public function endpoint_content( $type = false ) {
			wc_get_template( 'myaccount/quotes.php', array(), 'b2b-ecommerce-for-woocommerce', B2BE_TEMPLATE_DIR );
		}
		/**
		...
		public function add_view_quote_query_vars( $query_vars ) {
			$query_vars[ B2BE_VIEW_QUOTE_ENDPOINT ] = B2BE_VIEW_QUOTE_ENDPOINT;
			return $query_vars;
		}
		/**
		 * Account View Quote.
		...
		public function rfq_submit_quote() {
			...
			$quote = b2be_get_quote( $quote_id );
			$quote->submit_quote();
			$updated = b2be_get_quote( $quote_id );
			...
			if ( $product->is_type( 'simple' ) ) {
				...
				?>
					<a
						...
						href="?add-to-rfq=<?php echo wp_kses_post( $product->get_id() ); ?>"
						...
						class="button single_add_to_rfq_button_ajax alt added"
					>
