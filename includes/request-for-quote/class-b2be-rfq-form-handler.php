...
class B2BE_RFQ_Form_Handler {
	...
	public static function init() {
		...
		add_action( 'b2be_rfq_quote_marked_accepted', array( __CLASS__, 'populate_cart' ), 11, 2 );
	...
	private static function add_to_cart_handler_simple( $product_id, $ajax_quantity = 0 ) {
		...
		if ( $passed_validation ) {
			if ( false != WC()->rfq->add_to_cart( $product_id, $quantity ) ) {
				b2be_add_to_rfq_message( array( $product_id => $quantity ), true );
				return true;
			}
		}
		return false;
	...
	private static function add_to_cart_handler_grouped( $product_id ) {
		...
		if ( ! empty( $items ) ) {
			...
			if ( ! $was_added_to_cart && ! $quantity_set ) {
				...
				b2be_add_to_rfq_message( $added_to_cart );
	...
	public static function create() {
		...
		if ( isset( $_POST['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ) ) ) {
			...
			$quote_data = array(
				'post_title'  => sprintf( 'Quote from %s', $customer_first_name ),
		...
		if ( ! is_wp_error( $quote ) ) {
			...
			do_action( 'b2be_rfq_created', $quote );
			...
			$redirect_url = wc_get_endpoint_url( B2BE_QUOTE_ENDPOINT, '', wc_get_page_permalink( 'myaccount' ) );
	...
	public static function quote_need_revision() {
		...
		|| ! is_user_logged_in() || ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( filter_input( INPUT_GET, '_wpnonce' ), 'b2be-rfq-quote_action' ) ) {
			return;
		}
		if ( isset( $_GET['revise_quote'] ) ) {
			$quote = b2be_get_quote( absint( $_GET['revise_quote'] ) );
			...
			$quote->save();
			$quote = b2be_get_quote( $quote->get_id() );
			wc_add_notice( __( 'Quote is marked as Need Revision.', 'b2b-ecommerce' ) );
			do_action( 'b2be_rfq_quote_marked_need_revision', $quote->get_id(), $quote );
			$quotes_account_url = wc_get_endpoint_url( B2BE_QUOTE_ENDPOINT, '', wc_get_page_permalink( 'myaccount' ) );
			wp_safe_redirect( $quotes_account_url );
			exit;
		}
		if ( isset( $_GET['accept_quote'] ) ) {
			$quote = b2be_get_quote( absint( $_GET['accept_quote'] ) );

			if ( ! $quote->get_id() ) {
				return;
			}
			$quote->set_status( 'accepted' );
			do_action( 'b2be_rfq_quote_marked_accepted_email', $quote->get_id(), $quote );
			$quote->save();

			$accept_quotes_account_url = wc_get_endpoint_url( B2BE_QUOTE_ENDPOINT, '', wc_get_page_permalink( 'myaccount' ) );
			wp_safe_redirect( $accept_quotes_account_url );
			exit;
		}
		if ( isset( $_GET['accept_and_check_out'] ) ) {
			$quote = b2be_get_quote( absint( $_GET['accept_and_check_out'] ) );
			...
			$quote->save();
			$quote = b2be_get_quote( $quote->get_id() );
			do_action( 'b2be_rfq_quote_marked_accepted', $quote->get_id(), $quote );
			do_action( 'b2be_rfq_quote_marked_accepted_email', $quote->get_id(), $quote );
			$quotes_checkout_url = wc_get_endpoint_url( B2BE_QUOTE_ENDPOINT, '', wc_get_page_permalink( 'checkout' ) );
			wp_safe_redirect( $quotes_checkout_url );
			exit;
		}
		if ( isset( $_GET['reject_quote'] ) ) {
			$quote = b2be_get_quote( absint( $_GET['reject_quote'] ) );
			if ( ! $quote->get_id() ) {
				return;
			}
			...
			do_action( 'b2be_rfq_quote_marked_rejected', $quote->get_id(), $quote );
			$quotes_account_url = wc_get_endpoint_url( B2BE_QUOTE_ENDPOINT, '', wc_get_page_permalink( 'myaccount' ) );
			wp_safe_redirect( $quotes_account_url );
			exit;
		}
		if ( isset( $_GET['rfq_check_out'] ) ) {
			$quote = b2be_get_quote( absint( $_GET['rfq_check_out'] ) );
	...
	public static function populate_cart( $quote_id, $quote ) {
		...
		foreach ( $items as $item ) {
			...
			do_action( 'b2be_rfq_added_quote_item_to_cart', $product, $item, $quote_id );
		}
	}
	/**
