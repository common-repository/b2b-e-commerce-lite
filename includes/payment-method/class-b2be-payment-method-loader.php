	...
	class B2BE_Payment_Method_Loader {
		...
		public function enqueue_payment_method_backend_scripts( $page_id ) {
			if ( 'woocommerce_page_wc-settings' == $page_id ) {
				...
				if ( isset( $_GET['section'] ) && 'codup-payment-method' == $_GET['section'] ) {

					wp_enqueue_script( 'payment_method_settings_script', B2BE_ASSETS_DIR_URL . '/js/payment-method/admin/payment-method-settings.js', array( 'jquery' ), true );
					wp_localize_script(
