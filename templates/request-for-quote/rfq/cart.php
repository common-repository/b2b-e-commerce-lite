								...
								echo wp_kses_post(
									apply_filters(
										...
										sprintf(
											...
											esc_url( b2be_get_cart_remove_url( $cart_item_key ) ),
...
<form method="post" class="rfq-class">
	...
	<div>
		...
		?>
		<p><input type=<?php echo ( is_user_logged_in() && ! empty( $user_info->first_name ) ) ? 'hidden' : 'text'; ?> name="rfq_first_name" placeholder="First name" value="<?php echo ( is_user_logged_in() ) ? esc_attr( $user_info->first_name ) : ''; ?>" <?php echo ( is_user_logged_in() && ! empty( $user_info->first_name ) ) ? 'hidden="hidden"' : 'required="required"'; ?>/></p>
		<p><input type=<?php echo ( is_user_logged_in() && ! empty( $user_info->last_name ) ) ? 'hidden' : 'text'; ?> name="rfq_last_name" placeholder="Last name" value="<?php echo ( is_user_logged_in() ) ? esc_attr( $user_info->last_name ) : ''; ?>" <?php echo ( is_user_logged_in() && ! empty( $user_info->last_name ) ) ? 'hidden="hidden"' : 'required="required"'; ?>/></p>
		<p><input type=<?php echo ( is_user_logged_in() && ! empty( $user_info->user_email ) ) ? 'hidden' : 'email'; ?> name="rfq_email" placeholder="Email" value="<?php echo ( is_user_logged_in() ) ? esc_attr( $user_info->user_email ) : ''; ?>" <?php echo ( is_user_logged_in() ) ? 'hidden="hidden"' : 'required="required"'; ?>/></p>
		<p><textarea  name="rfq_message" required placeholder="Message" ></textarea></p>
