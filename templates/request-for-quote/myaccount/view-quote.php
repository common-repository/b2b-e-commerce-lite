...
printf(
	...
	'<mark class="order-status">' . wp_kses_post( b2be_get_quote_status_name( $quote->get_status() ) ) . '</mark>' // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
);
?>
</p>
<?php do_action( 'b2be_rfq_view_quote', $quote_id ); ?>
<?php
	...
	?>
<h2 class="woocommerce-column__title"><?php esc_html_e( 'Customer Chat', 'b2b-ecommerce' ); ?></h2>
<div style="padding:20px;background-color: rgba(0, 0, 0, 0.0125);">
	...
	<form method="post" id="customer_comment_form">
		...
		<input type="button" name="rfq_customer_comment_submit_button" id="rfq_customer_comment_submit_button" value="Send">
	</form>
</div>
